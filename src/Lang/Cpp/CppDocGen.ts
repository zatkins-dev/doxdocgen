...
import { getIndentation, replaceAll } from "../../util";
...
export class CppDocGen implements IDocGen {
    ...
    protected currentCursorIndex: number = 1;
    ...
    protected addCursor(line: string): string {
        if (line.includes(this.cfg.cursorTemplateReplace)) {
            return replaceAll(line, this.cfg.cursorTemplateReplace, () => `${this.currentCursorIndex++}`);
        }
        return line;
    }
    ...
    protected generateComment(): string {
        ...
        lines = templates.alignLines(lines);
        lines = lines.map(l => this.addCursor(l)).map((line) => `${this.cfg.C.commentPrefix}${line}`)
        this.insertFirstLine(lines);
        this.insertLastLine(lines);
    ...
    protected getFirstDoxdocCommandIndex(comment: string, baseCharacter: number): number {
